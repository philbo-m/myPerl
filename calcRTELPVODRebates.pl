...
open VOD , $vodFile or die "Cannot open VOD file [$vodFile] : $!";
<VOD>;
while ( <VOD> ) {
...
# Grab the RT and ELP rebate info.
# --------------------------------
my %poNameMap = ();
my %rtRebateMap = ();
my %elpRebateMap = ();
...
# Skip down to the section containing ELP Rebate info.
# ----------------------------------------------------
while ( <POFEE> ) {
	chomp;
	last if /BROKER NUMBER/;
...
#	my ( $po , $elpGrpID , $elpDesc , $elpRebate ) = /^(\d+),".*?","(.*?)","(.*?)",(.+)$/;

	push @{ $elpRebateMap{ $po } } , [ "$elpGrpID $elpDesc" , $elpRebate * -1 ];
}
close POFEE;
...
print "\"Name: 9, 10 & 11 - RT, ELP & VOD SYMBOL CREDITS (AMOUNTS WITHOUT TAX)\"\n";
print "Date: $dispDate\n\n";
...
$totQty += $totRT;
my $totELP = 0;
foreach my $po ( sort { $a <=> $b } keys %elpRebateMap ) {
	foreach my $poELPEntry ( @{ $elpRebateMap{ $po } } ) {
		my ( $elpDesc , $elpRebate ) = @$poELPEntry;
		printf "$date,$po,$poNameMap{ $po },$elpDesc,ELP,%.2f\n" , $elpRebate;
		$totELP += $elpRebate;
	}
}
printf "Total ELP,,,,Total ELP,%.2f\n" , $totELP;
