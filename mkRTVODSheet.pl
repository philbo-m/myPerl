...
sub usageAndExit {
	print STDERR "Usage : $scriptName -m yyyymm -t templateFile -p poRefFile -r billingFileDir -v vodSymCreditFile\n";
	exit 1;
}

my ( $rootDir , $vodFile , $poRefFile , $tmplFile );
...
usageAndExit if ( ( !$yyyymm || $yyyymm !~ /^201\d(0[1-9]|1[012])$/ ) || !$rootDir || !$vodFile || !$tmplFile || !$poRefFile );
