...
# --- NOTE - RT/Warrant and PRO awareness hacked in for Q1 2017 UAT ---
sub isRTWT {
	my ( $sym ) = @_;
...
# 	my $collapseMap = $FeeConv::collapseMap{ $oldNew };
	my $collapseMap = ( $what eq 'RTWT' ? $FeeConv::rtwtCollapseMap{ $oldNew } : 
						$what eq 'PRO' ? $FeeConv::proCollapseMap{ $oldNew } :
						$FeeConv::collapseMap{ $oldNew }
					);
...
# 	dbg ( "[$what] :\n" , Dumper ( \%revCollapseMap ) , "\n" );
	...
	foreach my $sym ( grep { $_ ne 'TOTAL' } sort keys %$feeMap ) {
		...
		next if ( $what eq 'RTWT' && !isRTWT ( $sym ) );
		
		foreach my $acctType ( keys %{ $$feeMap{ $sym } } ) {
		
			next if ( $what eq 'PRO' && $acctType ne 'PRO' );
...
#				--- HACK FOR 2018 Q1 ---
				if ( ( $subProd eq 'T_HI_RT_OL' || $subProd eq 'T_HI_MOO_RT' ) 
						&& $intlSymMap{ $sym } ) {
					$baseSubProd .= '_INTL';
				}
				
				if ( $what eq 'PRO' && $subProd =~ /T_LO_RT/ ) {
#					--- base subproduct will be 'T_LO_CLOB_T1_REG' ---
					my $avgPrc = $$feeNode{ 'TOTAL_VALUE' } / $$feeNode{ 'TOTAL_VOLUME' };
					if ( $avgPrc >= 0.10 ) {
						$baseSubProd =~ s/_T1/_T2/;
					}
				}
#				--- END HACK ---
...
#					--- HACK Q1 2018 - Move passive vol to active for PRO T_HI_MOC_RT ---
					if ( $what eq 'PRO' && $subProd eq 'T_HI_MOC_RT' ) {
						$$feeNode{ 'PASSIVE_VOLUME' } = 0;
						$$feeNode{ 'PASSIVE_TRADES' } = 0;
						$$feeNode{ 'PASSIVE_VALUE' } = 0;
					}
#					--- END HACK ---
...
# ---------------------------------------------------
collapse ( \%newFeeMap , \@valKeys , \@noFeeValKeys , 'NEW' , 'RTWT' );

# Collapse the PRO RT subproducts (Q1 2018 ONLY).
# -----------------------------------------------
my $intlSymList = getIntlSyms ( \%oldFeeMap );
# dbg ( "[" , join ( " , " , sort @$intlSymList ) , "]" );
collapse ( \%newFeeMap , \@valKeys , \@noFeeValKeys , 'NEW' , 'PRO' , $intlSymList );
