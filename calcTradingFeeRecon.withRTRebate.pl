...
sub getTrdDays {
	...
	my $dailyDir = sprintf ( "$rootDir/daily/%d_%02d" , ${yyyy} , ${mm} );
	my $trdDayCnt;
	if ( -d $dailyDir ) {
		my @dailyFiles = glob ( "$dailyDir/*.csv" );
		$trdDayCnt = scalar @dailyFiles;
	}
	else {
		print STDERR "Unable to determine trading day count in directory [$dailyDir].\n";
		print STDERR "Enter number of trading days in ${yyyy}_${mm} : ";
		my $trdDayCnt = <>;
		chomp $trdDayCnt;
	}
...
sub showProducts {
	my ( $thisDispMon , $prevDispMon , $currPOSumFile , $prevPOSumFile , 
		$currQtyMap , $prevQtyMap , $currTrdDays , $prevTrdDays , $subProdMap ) = @_;
...
#	----------------------------------------------
	...
	foreach my $product ( keys %currQtyByProduct ) {
		...
		printf STDERR "Current non-billable vol [$product]: [$currQtyByProduct{ $product }{ 'VOL' }]\n";
	...
	foreach my $product ( keys %prevQtyByProduct ) {
		...
		printf STDERR "Previous non-billable vol [$product]: [$prevQtyByProduct{ $product }{ 'VOL' }]\n";
	...
	$currTotFee /= $currTrdDays;
	$currTotVol /= $currTrdDays;
	$prevTotFee /= $prevTrdDays;
	$prevTotVol /= $prevTrdDays;
	printf "Daily Totals,%.2f,%.2f,%.3f,%.2f,%.0f,%.0f,%.3f,%.0f\n" , 
			$currTotFee , $prevTotFee , mkPctChg ( $currTotFee , $prevTotFee ) , $currTotFee - $prevTotFee ,
			$currTotVol , $prevTotVol , mkPctChg ( $currTotVol , $prevTotVol ) , $currTotVol - $prevTotVol;
}
...
my ( $yyyy , $mm ) = ( $yyyymm =~ /(....)(..)/ );
my $currTrdDays = getTrdDays ( $rootDir , $yyyy , $mm );
...
if ( $byYear ) {
	...
	( $prevYYYY , $prevMM ) = Util::prevMonth ( $yyyy , $mm ); 
}
my $prevYYYYMM = sprintf ( "%04d%02d" , $prevYYYY , $prevMM );
my $prevTrdDays = getTrdDays ( $rootDir , $prevYYYY , $prevMM );
...
showProducts ( $thisDispMon , $prevDispMon , $currReconFile , $prevReconFile , 
				$currQtyMap , $prevQtyMap , $currTrdDays , $prevTrdDays , 
				\%TSXProdMap::subProdMap );
...
my $currELPRebate = ( $currReconFile->{elpSection} ? $currReconFile->{elpSection}->val ( {} , "ELP REBATE" ) : 0 );
my $prevELPRebate = ( $prevReconFile->{elpSection} ? $prevReconFile->{elpSection}->val ( {} , "ELP REBATE" ) : 0 );
if ( $currELPRebate || $prevELPRebate ) {
	printf "ELP Rebate,%.2f,%.2f,%.3f,%.2f\n" , $currELPRebate , $prevELPRebate , 
				mkPctChg ( $currELPRebate , $prevELPRebate ) , $currELPRebate - $prevELPRebate;
}
